...
new_fcts <- c(
  ...
  tornado = c("torn", "nado"),
  tsunami = c("tsun"),
  stormy = c("storm*", "thunder*", "tstm", "rain", "precip", "hurric"),
  lightning = c("lightn"),
...
for (gigio in new_fcts) {
  fct <- str_subset(head(sel_data$event, 50), gigio)
  results <- c(results, setNames(fct, names(new_fcts)[counter]))
  counter <- counter + 1
}
